...
void main() { do
{
    ...
    int temp_32;
    bool temp_33;
    temp_0 = fma(clamp(in_attr5.y, 0.0, 1.0), 0.0 - _sysCustomShaderReservedUniformBlockParam.data[3].x, _sysCustomShaderReservedUniformBlockParam.data[3].x);
    temp_1 = exp2(log2(abs(fma(exp2(log2(abs(clamp(0.0 - in_attr6.z + 1.0 + in_attr7.w, 0.0, 1.0) * texture(sysTextureSampler0, vec2(in_attr2.x, in_attr2.y)).x)) * _sysCustomShaderReservedUniformBlockParam.data[3].z), texture(sysTextureSampler2, vec2(in_attr3.x, in_attr3.y)).x, exp2(log2(abs(clamp(0.0 - in_attr6.z + 1.0 + in_attr7.w, 0.0, 1.0) * texture(sysTextureSampler0, vec2(in_attr2.x, in_attr2.y)).x)) * _sysCustomShaderReservedUniformBlockParam.data[3].z)) * 0.5)) * 0.675000012) + 0.0 - fma(in_attr0.w, -0.5, temp_0) + -1.0;
    ...
    temp_5 = clamp(temp_4 + -0.0, 0.0, 1.0) * in_attr5.x;
    ...
    temp_8 = in_attr1.z;
    temp_9 = in_attr1.x;
    temp_10 = clamp(0.0 - in_attr6.y + 1.0, 0.0, 1.0) * _sysCustomShaderReservedUniformBlockParam.data[4].w;
